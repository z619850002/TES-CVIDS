...
public:
	...
	bool Aligned(int nClientID){
		return this->m_dAligned[nClientID];
	}
...
public:
	...
	bool m_bUpdateGlobalMap;
	std::mutex m_mUpdateGlobalMapMutex;

	Sophus::SE3 GetTransformation(int nClientID){
    Sophus::SE3 mTransform_wr(m_dLocalRotation_wr[nClientID], m_dLocalTranslation_wr[nClientID]);
    Sophus::SE3 mDrift_wr(m_dRotationDrift[nClientID], m_dTranslationDrift[nClientID]);
    Sophus::SE3 mT = mDrift_wr * mTransform_wr;
    return mT;
    // Eigen::Matrix3d mGlobalRotation_wi = m_dLocalRotation_wr[nClientID] * mLocalRotation_wi;
	}

	void SetUpdateGlobalMap(bool bFlag){
		this->m_mUpdateGlobalMapMutex.lock();
		this->m_bUpdateGlobalMap = bFlag;
		this->m_mUpdateGlobalMapMutex.unlock();
	}

	bool GetUpdateGlobalMap()
	{
		return this->m_bUpdateGlobalMap;
	}
